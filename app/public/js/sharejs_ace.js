(function() {
  ...
  applyToShareJS = function(editorDoc, delta, doc) {
    ...
    switch (delta.action) {
      case 'insertText':
        context.insert(pos, delta.text);
        break;
      case 'removeText':
        context.remove(pos, delta.text.length);
        break;
      case 'insertLines':
      ...
      case 'removeLines':
        text = delta.lines.join('\n') + '\n';
        context.remove(pos, text.length);
        break;
      default:
  ...
  window.sharejs.Doc.prototype.attach_ace = function(editor, keepEditorContents) {
    ...
    if (keepEditorContents) {
      context.remove(0, doc.getSnapshot().length);
      context.insert(0, editorDoc.getValue());
    } else {
      editorDoc.setValue(doc.getSnapshot());
    }
    check();
    suppress = false;
    ...
    context.onInsert = function(pos, text) {
      if (editorDoc.getValue() === doc.getSnapshot()) return check();
      suppress = true;
      editorDoc.insert(offsetToPos(pos), text);
      suppress = false;
      return check();
    };
    context.onRemove = function(pos, text) {
      if (editorDoc.getValue() === doc.getSnapshot()) return check();
      var range;
      suppress = true;
